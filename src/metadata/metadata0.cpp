...
namespace flac_bindings {
    ...
    static Value asyncImpl(const Env& env, const char* name, std::function<FLAC__StreamMetadata*()> impl) {
    ...
    static Value getStreaminfoAsync(const CallbackInfo& info) {
        ...
        return asyncImpl(
            info.Env(),
            "flac_bindings::getStreaminfoAsync",
            [path] () {
                FLAC__StreamMetadata metadata;
                FLAC__bool ret = FLAC__metadata_get_streaminfo(path.c_str(), &metadata);
    ...
    static Value getCuesheetAsync(const CallbackInfo& info) {
        ...
        return asyncImpl(
            info.Env(),
            "flac_bindings::getCuesheetAsync",
            [path] () {
                FLAC__StreamMetadata* metadata = nullptr;
                FLAC__metadata_get_cuesheet(path.c_str(), &metadata);
                return metadata;
    ...
    static Value getPicture(const CallbackInfo& info) {
        std::string path = stringFromJs(info[0]);
        auto type = maybeNumberFromJs<FLAC__StreamMetadata_Picture_Type>(info[1])
            .value_or((FLAC__StreamMetadata_Picture_Type) -1);
    ...
    Object initMetadata0(const Env& env) {
        ...
        metadata0.DefineProperties({
            ...
            PropertyDescriptor::Function(env, metadata0, "getStreaminfoAsync", &getStreaminfoAsync, attrs),
