...
namespace flac_bindings {
    ...
    static Value vorbiscommentEntryValueIsLegal(const CallbackInfo& info) {
        auto string = stringFromJs(info[0]);
        FLAC__bool ret = FLAC__format_vorbiscomment_entry_value_is_legal((FLAC__byte*) string.c_str(), string.length());
        return booleanToJs(info.Env(), ret);
    }

    static Value vorbiscommentEntryIsLegal(const CallbackInfo& info) {
        auto string = stringFromJs(info[0]);
        FLAC__bool ret = FLAC__format_vorbiscomment_entry_is_legal((FLAC__byte*) string.c_str(), string.length());
        return booleanToJs(info.Env(), ret);
    }
    ...
    static Value seektableSort(const CallbackInfo& info) {
        ...
        uint32_t numberOfPlaceholdersConvertedToTemplates = FLAC__format_seektable_sort(&metadata->data.seek_table);
    ...
    static c_enum::DefineReturnType createEntropyCodingMethodTypeEnum(const Env& env) {
        Object obj1 = Object::New(env);
        Object obj2 = Object::New(env);
        c_enum::defineValue(obj1, obj2, "PARTITIONED_RICE", FLAC__ENTROPY_CODING_METHOD_PARTITIONED_RICE);
        c_enum::defineValue(obj1, obj2, "PARTITIONED_RICE2", FLAC__ENTROPY_CODING_METHOD_PARTITIONED_RICE2);
        return std::make_tuple(obj1, obj2);
    }
    ...
    static c_enum::DefineReturnType createPictureTypeEnum(const Env& env) {
        ...
        c_enum::defineValue(obj1, obj2, "FILE_ICON_STANDARD", FLAC__STREAM_METADATA_PICTURE_TYPE_FILE_ICON_STANDARD);
        ...
        c_enum::defineValue(obj1, obj2, "LEAFLET_PAGE", FLAC__STREAM_METADATA_PICTURE_TYPE_LEAFLET_PAGE);
        ...
        c_enum::defineValue(obj1, obj2, "RECORDING_LOCATION", FLAC__STREAM_METADATA_PICTURE_TYPE_RECORDING_LOCATION);
        c_enum::defineValue(obj1, obj2, "DURING_RECORDING", FLAC__STREAM_METADATA_PICTURE_TYPE_DURING_RECORDING);
        c_enum::defineValue(obj1, obj2, "DURING_PERFORMANCE", FLAC__STREAM_METADATA_PICTURE_TYPE_DURING_PERFORMANCE);
        c_enum::defineValue(obj1, obj2, "VIDEO_SCREEN_CAPTURE", FLAC__STREAM_METADATA_PICTURE_TYPE_VIDEO_SCREEN_CAPTURE);
        c_enum::defineValue(obj1, obj2, "FISH", FLAC__STREAM_METADATA_PICTURE_TYPE_FISH);
        c_enum::defineValue(obj1, obj2, "ILLUSTRATION", FLAC__STREAM_METADATA_PICTURE_TYPE_ILLUSTRATION);
        c_enum::defineValue(obj1, obj2, "BAND_LOGOTYPE", FLAC__STREAM_METADATA_PICTURE_TYPE_BAND_LOGOTYPE);
        c_enum::defineValue(obj1, obj2, "PUBLISHER_LOGOTYPE", FLAC__STREAM_METADATA_PICTURE_TYPE_PUBLISHER_LOGOTYPE);
    ...
    Object initFormat(const Env& env) {
        ...
        format.DefineProperties({
            ...
            PropertyDescriptor::Function(env, format, "vorbiscommentEntryNameIsLegal", &vorbiscommentEntryNameIsLegal),
            PropertyDescriptor::Function(env, format, "vorbiscommentEntryValueIsLegal", &vorbiscommentEntryValueIsLegal),
            PropertyDescriptor::Function(env, format, "vorbiscommentEntryIsLegal", &vorbiscommentEntryIsLegal),
