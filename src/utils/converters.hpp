...
namespace flac_bindings {
    ...
    static inline std::optional<T> maybeNumberFromJs(const Napi::Value& value) {
        if(value.IsNumber()) {
            ...
            if(!lossless) {
                throw Napi::TypeError::New(bigNum.Env(), "Number "s + bigNum.ToString().Utf8Value() + " is too big for be converted into native number"s);
            }
    ...
    static inline std::optional<T> maybeNumberFromJs(const Napi::Value& value) {
        if(value.IsNumber()) {
            ...
            if(intValue < 0) {
                throw Napi::RangeError::New(value.Env(), "Number "s + std::to_string(intValue) + " is negative - expected unsigned value");
            }
    ...
    static inline T numberFromJs(const Napi::Value& value) {
        ...
        if(maybeNumber == std::nullopt) {
            throw Napi::TypeError::New(value.Env(), "Expected "s + value.ToString().Utf8Value() + " to be number or bigint"s);
        }
    ...
    >
    static inline Napi::Value numberToJs(const Napi::Env& env, T enumValue, bool forceBigInt = false) {
    ...
    static inline std::string stringFromJs(const Napi::Value& value) {
        ...
        if(maybeString == std::nullopt) {
            throw Napi::TypeError::New(value.Env(), "Expected "s + value.ToString().Utf8Value() + " to be string"s);
        }
    ...
    static inline std::vector<Type> arrayFromJs(
        ...
        if(!value.IsArray()) {
            throw Napi::TypeError::New(value.Env(), "Expected "s + value.ToString().Utf8Value() + " to be Array");
        }
    ...
    static inline void functionIntoRef(Napi::FunctionReference& ref, const Napi::Value& value) {
        if(!maybeFunctionIntoRef(ref, value)) {
            throw Napi::TypeError::New(value.Env(), "Expected "s + value.ToString().Utf8Value() + " to be function");
        }
    }
