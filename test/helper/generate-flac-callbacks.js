...
const generateFlacCallbacks = {
    ...
    flacio: async (path, flags) => {
        ...
        const read = async (buffer, sizeOfItem, numberOfItems) => {
            const { bytesRead: bytes } = await fh.read(buffer, 0, sizeOfItem * numberOfItems, position);
            position += bytes;
            return bytes / sizeOfItem;
        };

        const write = async (buffer, sizeOfItem, numberOfItems) => {
            const { bytesWritten: bytes } = await fh.write(buffer, 0, sizeOfItem * numberOfItems, position);
            position += bytes;
            return bytes / sizeOfItem;
        };
