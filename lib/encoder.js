...
const convertToInt32Buffer = (chunk, self) => {
    ...
    const buffer = flac.fns.convertSampleFormat({ inBps: inBPS, buffer: chunk, samples });
...
const setOptions = (self, options) => {
    ...
    self._inputAs32 = (options.inputAs32 != null) ?
        !!options.inputAs32 :
        ...
        if(!self._encoderInit) {
            ...
            try {
                if(self._oggStream) {
                    ...
                    await self._initOggEncoder();
                } else {
                    self._debug('Initializing encoder');
                    await self._initEncoder();
                }
            } catch(error) {
                const initStatus = error.status || -1;
                const initStatusString = error.statusString || 'Unknown';
                self._debug(`Failed initializing encoder: ${initStatus} ${initStatusString}`);
                throw error;
            }
