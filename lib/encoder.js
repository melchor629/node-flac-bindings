...
const setOptionsToEncoder = (self) => {
    ...
    if(typeof self._minResidualPartitionOrder === 'number') {
        self._enc.setMinResidualPartitionOrder(self._minResidualPartitionOrder);
    }
    if(typeof self._maxResidualPartitionOrder === 'number') {
        self._enc.setMaxResidualPartitionOrder(self._maxResidualPartitionOrder);
    }
    ...
    async _transform(chunk, _, callback) {
        if(!this._encoderInit) {
            try {
                ...
                if(this._oggStream) {
                    await this._enc.initOggStreamAsync(null, this._writeFlac.bind(this), null, null, null);
                } else {
                    await this._enc.initStreamAsync(this._writeFlac.bind(this), null, null, null);
                }
    ...
    async _write(chunk, _, callback) {
        if(!this._encoderInit) {
            try {
                setOptionsToEncoder(this);
                if(this._oggStream) {
                    await this._enc.initOggFileAsync(this._file, null);
                } else {
                    await this._enc.initFileAsync(this._file, null);
                }
                this._encoderInit = true;
            } catch(err) {
                callback(err);
