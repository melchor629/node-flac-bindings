...
const setOptions = (self, options) => {
    ...
    self._inputAs32 = Boolean(options.inputAs32) || false;
    self._samplerate = Number(options.samplerate) || 44100;
    self._oggSerialNumber = Number(options.oggSerialNumber) || null;
    self._totalSamplesEstimate = Number(options.totalSamplesEstimate);
    self._compressionLevel = Number(options.compressionLevel);
    ...
    self._maxLpcOrder = Number(options.maxLpcOrder);
    self._qlpCoeffPrecision = Number(options.qlpCoeffPrecision);
    self._doQlpCoeffPrecSearch = options.doQlpCoeffPrecSearch;
    self._doExhaustiveModelSearch = options.doExhaustiveModelSearch;
    self._minResidualPartitionOrder = Number(options.minResidualPartitionOrder);
    self._maxResidualPartitionOrder = Number(options.maxResidualPartitionOrder);
    self._riceParameterSearchDist = Number(options.riceParameterSearchDist);
};
