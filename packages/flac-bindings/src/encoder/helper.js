...
class BaseEncoder {
  ...
  setOptionsToEncoder(options) {
    ...
    if (options.numThreads) {
      if (typeof options.numThreads === 'boolean') {
        this._builder.setNumThreads(cpus().length)
      } else {
        this._builder.setNumThreads(options.numThreads)
      }
    }
  }
   ...
   * @param {Buffer[]} chunks A chunk of data
